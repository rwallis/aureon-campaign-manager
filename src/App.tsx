...
export default function App() {
  ...
  const applyImprovement = (label: string, improvement?: number) => {
    ...
    }[panelKey as keyof typeof goalKpiMap]) {
      const info = {
        Audience: {
          text: "Refine audience targeting towards high converters to reduce CPA.",
          improvement: 8,
  ...
  function DecisionEngineMarketerUI() {
    ...
    // Signals & Access types, seed and state (new)
    type AccessStatus = "connected" | "partial" | "missing" | "requested";
    type AccessItem = { key: string; label: string; status: AccessStatus; needs: string[] };
    const initialAccess: AccessItem[] = [
      { key: "ad_platforms", label: "Ad Platforms / DSP Reports", status: "connected", needs: [
        "Spend/Impr/Clicks/Convs by campaign/audience/creative",
        "Auction diagnostics (if available)"
      ]},
      { key: "ad_server", label: "Ad Server (CM360, Kevel, Mediaocean)", status: "partial", needs: [
        "Cross-channel conversions for reconciliation"
      ]},
      { key: "cdp_id", label: "CDP / Identity Graph", status: "missing", needs: [
        "Match rates by partner",
        "Audience sync status & sizes"
      ]},
      { key: "analytics", label: "Web/App Analytics (GA4/Adobe/Mixpanel)", status: "connected", needs: [
        "Bounce/Exit/Depth by landing URL",
        "Post-click conversions"
      ]},
      { key: "rum", label: "Pixels / Web Vitals", status: "partial", needs: [
        "LCP/CLS/TTFB by landing page"
      ]},
      { key: "quality", label: "Viewability/Fraud/Safety (DV/IAS/MOAT)", status: "missing", needs: [
        "Viewability%","IVT rate","Brand-safety block rate"
      ]},
      { key: "plan", label: "Plan / Budget (Aureon Plan table)", status: "connected", needs: [
        "Daily planned spend by partner"
      ]},
    ...
    // access state + ai requests
    const [access, setAccess] = useState<AccessItem[]>(initialAccess);
    const [aiRequests, setAiRequests] = useState<{ key: string; label: string; reason: string }[]>([]);

    function requestAccess(item: AccessItem) {
      setAccess(prev => prev.map(x => x.key === item.key ? { ...x, status: "requested" } : x));
      const reason = `Needed for: ${item.needs.join("; ")}`;
      setAiRequests(prev => prev.some(r => r.key === item.key) ? prev : [...prev, { key: item.key, label: item.label, reason }]);
    }

    function markConnected(key: string) {
      setAccess(prev => prev.map(x => x.key === key ? { ...x, status: "connected" } : x));
      setAiRequests(prev => prev.filter(r => r.key !== key));
    }

    function StatusBadge({ status }: { status: AccessStatus }) {
      const map: Record<AccessStatus, { text: string; cls: string }> = {
        connected: { text: "Connected", cls: "bg-green-100 text-green-800" },
        partial: { text: "Partial", cls: "bg-amber-100 text-amber-800" },
        missing: { text: "Missing", cls: "bg-rose-100 text-rose-800" },
        requested: { text: "Requested", cls: "bg-purple-100 text-purple-800" },
      };
      ...
      <div className="space-y-6">
        ...
        <section className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="rounded-2xl bg-white p-5 shadow lg:col-span-2">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold">Signals & Access (AI needs)</h2>
              <Badge className="bg-gray-100 text-gray-700" title="Read-only scopes are sufficient">read-only</Badge>
            </div>
            <ul className="space-y-3">
              {access.map((a) => (
                <li key={a.key} className="flex items-center justify-between rounded-xl border p-3">
                  <div>
                    <div className="font-medium">{a.label}</div>
                    <div className="text-xs text-gray-600">Requires: {a.needs.join(", ")}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <StatusBadge status={a.status} />
                    {a.status !== "connected" && (
                      <button onClick={() => requestAccess(a)} className="rounded-lg bg-purple-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-purple-700">
                        Request access
                      </button>
                    )}
                    {a.status !== "connected" && (
                      <button onClick={() => markConnected(a.key)} className="rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-800 hover:bg-gray-200">
                        Mark connected
                      </button>
                    )}
                  </div>
          ...
          <div className="rounded-2xl bg-white p-5 shadow">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold">AI Requests for Access</h2>
              <Badge className="bg-blue-100 text-blue-800">{aiRequests.length}</Badge>
            </div>
            {aiRequests.length === 0 ? (
              <div className="rounded-xl border border-dashed p-6 text-center text-sm text-gray-500">No pending requests.</div>
            ) : (
              <ul className="space-y-3">
                {aiRequests.map((r) => (
                  <li key={r.key} className="rounded-xl border p-3">
                    ...
                    <div className="mt-2 flex gap-2">
                      <button onClick={() => markConnected(r.key)} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-emerald-700">
                        Connect
                      </button>
                      <button onClick={() => setAiRequests((prev) => prev.filter((x) => x.key !== r.key))} className="rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-800 hover:bg-gray-200">
                        Dismiss
                      </button>
                    </div>
                  </li>
            ...
            <p className="mt-4 text-xs text-gray-500">Granting access unlocks signals like identity coverage, viewability, and forecast awareness.</p>
          </div>
        </section>
